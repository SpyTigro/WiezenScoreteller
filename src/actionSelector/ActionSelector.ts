...
export default class ActionSelector {
        ...
        for (let i = 0; i < this.game.roundTypes.length; i++) {
            ...
            RadioEl.addEventListener("input", () => {
                ...
                allLabels.forEach((l) => {
                    l.className = "check";
                    if (
                        (document.getElementById(l.htmlFor) as HTMLInputElement)?.checked
                    ) {
                        l.className = "checked";
                    }
                    if (RadioEl.checked) {
    ...
    private makePlayerCheckDiv(name: string, i: number): HTMLElement {
        ...
        if (this.game.players.length == 5 && i != this.game.deler)
            checkEl.addEventListener("change", (e) => {
                if (i > this.game.deler) this.checkedP[i - 1] = checkEl.checked;
                else this.checkedP[i] = checkEl.checked;
    ...
    private renderSlagenSelector() {
        ...
        if (type.teamed) {
            ...
            if (this.game.players.length == 5)
                selectedPlayers = this.game.players.filter((_, i) =>
                    i == this.game.deler
                        ? false
                        : i < this.game.deler
                            ? this.checkedP[i]
            ...
            selectedPlayers.forEach((p, i) => {
                this.slagenSelectorDiv.appendChild(
                    this.makeSlagenSlider(i, `Slagen voor ${p}: `),
                );
            });
        }
    }
