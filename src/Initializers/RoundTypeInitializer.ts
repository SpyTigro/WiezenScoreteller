...
export class RoundTypeInitializer {
    ...
    addRoundTypeSetter(id: number, roundType?: RoundType): HTMLDivElement {
        ...
        OptionBtnEl.addEventListener("click", () => {
            if (OptionDiv.hidden) {
                ...
                OptionBtnEl.innerText = "Options V";
            } else {
                OptionDiv.hidden = true;
                OptionBtnEl.innerText = "Options >";
            }
        ...
        addAttributeBtn.addEventListener("click", () => {
            ...
            const option = document.getElementById(
                `${this.htmlId}-attributeSelectOption${id}${i}`,
            ) as HTMLOptionElement;
            if (option) {
                option.disabled = true;

                const remainingVisibleOptionIndex = Array.from(
                    attributeSelect.options,
                ).findIndex(
                    (selectOption, optionIndex) =>
                        optionIndex !== i && !selectOption.disabled,
                );

                attributeSelect.selectedIndex =
                    remainingVisibleOptionIndex >= 0 ? remainingVisibleOptionIndex : -1;
            } else return;
        ...
        OptionDiv.appendChild(
            this.numInWithLabel(id, "Threshold", roundType?.threshold),
        );
    ...
    private addAttributeSetter(
        id: number,
        option: HTMLOptionElement,
        OptionDiv: HTMLDivElement,
        roundType?: RoundType,
    ) {
        const attributeDiv = this.rowDiv(
            `${this.htmlId}-attribute${option.innerText}${id}`,
        );
        ...
        const attributeSetterDiv = this.attributeSetterDiv(
            id,
            option.innerText,
            roundType,
        );
    ...
    private attributeSetterDiv(
        ...
        else return this.checkWithLabel(id, attribute, value);
    }

    private numInWithLabel(
        id: number,
        name: string,
        value: number = 0,
    ): HTMLDivElement {
