export default class ActionSelector {
    ...
    get roundResult() {
        if (this.slagen.reduce((total, cur) => total + cur, 0) > 13)
            throw new Error("teveel slagen");
        return {
            teamA: this.checkedP,
            hits: this.slagen,
    ...
    makeTypeSelector() {
        ...
        for (let i = 0; i < this.game.roundTypes.length; i++) {
            ...
            if (RadioEl.checked)
                radiolabelEl.className = "checked";
            else
                radiolabelEl.className = "check";
            ...
            RadioEl.addEventListener("input", () => {
                ...
                allLabels.forEach((l) => {
                    var _a;
                    l.className = "check";
                    if ((_a = document.getElementById(l.htmlFor)) === null || _a === void 0 ? void 0 : _a.checked) {
                        l.className = "checked";
                    }
                    if (RadioEl.checked) {
                        this.checkedM = i;
    ...
    makePlayerCheckDiv(name, i) {
        ...
        checkEl.value = name;
        let ilabelEl = document.createElement("label");
    ...
    renderSlagenSelector() {
        ...
        if (type.teamed) {
            ...
            if (this.game.players.length == 5)
                selectedPlayers = this.game.players.filter((_, i) => i == this.game.deler
                    ? false
                    : i < this.game.deler
                        ? this.checkedP[i]
                        : this.checkedP[i - 1]);
    ...
    setSlagenLength(length) {
        if (this.slagen.length > length) {
            ...
            this.slagen = [
                ...this.slagen,
                ...new Array(length - this.slagen.length).fill(0),
            ];
        }
    }
    makeSlagenSlider(i, label) {
